...
let paraphrase_modifiers  modif =
        match modif with
...
let paraphrase_if_then cond1 cond2 =
...
let aiguillage_phrase l = match  exists_Operator l, exists_Predicate l, exists_Modifier_pp l, exists_Relation l with
|  false, true, false,false   -> let verbes = List.filter is_Predicate  l in List.map paraphrase_from_verb verbes
...
|  false, false, false,false  -> ["Phrase nominale"]
...
|  true , _, false, _  -> 
                ...
                let gereOP operator = match operator with
                                        | Operator2(Imply, FullDRS(dom,cond1), FullDRS(dom2,cond2)) -> paraphrase_if_then cond1 cond2
                                        |               Operator2(_,_,_) -> failwith "op pas géré" 
                                        | _ -> failwith "aiguillage_phrase : Pas opérateur" in
                List.map gereOP operators
